...
import { Users, Plus, Lock, Unlock, UserX, RefreshCw, Search } from "lucide-react";

// Admin staff accounts use /api/admin/staff-accounts (not /api/admin/admin/staff-accounts)
async function adminStaffCall(path: string, options: RequestInit = {}) {
  const res = await apiRequest(`/api/admin/staff-accounts${path}`, options);
...
export default function StaffAccountsCmsPage() {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminStaffCall(`?${params}`);
      setAccounts(res.data ?? res ?? []);
    } catch (err: unknown) {
      showToast("error", err instanceof Error ? err.message : "Failed to load accounts.");
    } finally {
      setLoading(false);
    }
  }, [search, filterStatus]);
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    try {
      const res = await adminStaffCall("/", {
        method: "POST",
      ...
      setForm({ ...EMPTY_FORM });
      await loadAccounts();
      showToast("success", "Account provisioned successfully.");
    } catch (err: unknown) {
  ...
  async function doAction(id: number, type: "lock" | "unlock" | "deactivate" | "reset-password") {
    setActionLoading(id);
    try {
      const res = await adminStaffCall(`/${id}/${type}`, {
        method: "POST",
        body: JSON.stringify({}),
        headers: { "Content-Type": "application/json" },
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Provision, manage, and audit staff portal accounts.</p>
        </div>
              ...
              {accounts.map(acc => (
                <tr key={acc.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors" data-testid={`account-row-${acc.id}`}>
                  <td className="px-4 py-3">
                    <p className="font-medium text-gray-900">{acc.title ? `${acc.title} ` : ""}{acc.name}</p>
                  ...
                  <td className="px-4 py-3"><span className="text-xs text-gray-500">{acc.last_login_at ? formatDateTime(acc.last_login_at) : "—"}</span></td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5 justify-end">
                      ...
                      {acc.status !== "inactive" && (
                        <button onClick={() => doAction(acc.id, "deactivate")} disabled={actionLoading === acc.id}
                          title="Deactivate account" data-testid={`btn-deactivate-${acc.id}`}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="font-bold text-gray-900">Provision New Staff Account</h2>
